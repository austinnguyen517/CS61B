public class ArrayDeque<T> {
    /* Class definition of ArrayDeque*/
    ...
    public void addFirst(T item){
        ...
        /*This adds the element at start MINUS ONE since start keeps track of the first element filled*/
    ...
    public T get(int index){
        ...
        int new_ind = (first + index) % items.length;
        if (this.isEmpty()){
            return null;
        }
        if ((last > first && !(new_ind >= first && new_ind < last)) || (first > last && (new_ind < first && new_ind >= last))){
            /*If last is larger than first and the new index is not between it, it is not in the deque*/
            /* or if first is larger than last and the new index IS in between the two nodes */
            return null;
        }

        return items[new_ind];
    }
}